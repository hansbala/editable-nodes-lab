...
describe('Unit Test: Create Link', () => {
  ...
  test('fails to insert link with duplicate id', async () => {
    ...
    const invalidLink: ILink = makeILink('link1', 'anchor1', 'anchor2')
  ...
  test('fails to insert link with duplicate anchorIds', async () => {
    const invalidLink: ILink = makeILink('link1', 'anchor1', 'anchor1')
    const invalidResponse = await linkGateway.createLink(invalidLink)
    expect(invalidResponse.success).toBeFalsy()
  })
  ...
  test('fails to insert link when linkId is not defined', async () => {
    const invalidLink = makeILink(undefined, 'anchor1', 'anchor2')
    const response = await linkGateway.createLink(invalidLink)
    expect(response.success).toBeFalsy()
  })

  test('fails to insert link when anchor1Id is not defined', async () => {
    const invalidLink = makeILink('link1', undefined, 'anchor2')
    const response = await linkGateway.createLink(invalidLink)
    expect(response.success).toBeFalsy()
  })

  test('fails to insert link when anchor2Id is not defined', async () => {
    const invalidLink = makeILink('link1', 'anchor1', undefined)
    const response = await linkGateway.createLink(invalidLink)
    expect(response.success).toBeFalsy()
  })
